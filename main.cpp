...
// emcc -o raylib_game.html main.cpp -Wall -std=c++14 -D_DEFAULT_SOURCE -Wno-missing-braces -Wunused-result -Os -I. -I C:/raylib/raylib/src -I C:/raylib/raylib/src/external -L. -L C:/raylib/raylib/src -s USE_GLFW=3 -s ASYNCIFY -s TOTAL_MEMORY=67108864 -s FORCE_FILESYSTEM=1 --preload-file assets --shell-file C:/raylib/raylib/src/shell.html C:/raylib/raylib/src/web/libraylib.a -DPLATFORM_WEB -s 'EXPORTED_FUNCTIONS=["_free","_malloc","_main"]'-s EXPORTED_RUNTIME_METHODS=ccall
...
int main(void)
{
    ...
    atexit([]() {
        ...
        UnloadTexture(textureFlip3);
    });
    #endif


    texture = LoadTexture("assets/pfiddyone2.png");
    // textureFlip = LoadTexture("pfiddyone2flip.png");
    // textureFlip2 = LoadTexture("pfiddyone2flip2.png");
    textureFlip3 = LoadTexture("assets/pfiddyone2flip3.png");
    InitGame();
...
void InitGame(void)
{
    ...
    if (texture.id == 0) {
        std::cout << "Failed to load texture pfiddyone2.png" << '\n';
        // Handle error - maybe set a default color to use instead
    }
...
void DrawGame()
{
    BeginDrawing();
        ...
        if (!gameOver)
        {
            ...
            DrawText(text.c_str(), (GetScreenWidth()-100), 20, 20, BLACK); 
            text = "Bullets: " + std::to_string(myPlane.dur.count());
            DrawText(text.c_str(), (GetScreenWidth()-100), 120, 20, BLACK);
